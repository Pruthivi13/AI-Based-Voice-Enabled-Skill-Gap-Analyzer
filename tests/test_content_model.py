...
try:
    ...
    info(f"Transformer model unavailable: {e}")
    similarity = semantic_similarity(
        "REST uses stateless HTTP resources",
        "REST is stateless and uses HTTP resources",
    )
    check(similarity > 0.25, f"Local semantic scorer available (similarity={similarity})")
...
for pair in test_pairs:
    if MODEL_AVAILABLE:
        ...
        prediction = hybrid_score(pair["question"], pair["answer"], pair["question"])["model_label"]
    check(
