...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 backdrop-blur-md border-b transition-colors duration-300 ${
      isDark
        ? 'bg-dark-900/80 border-dark-700/60'
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
        ...
        <button onClick={() => navigate('/')} className="flex items-center gap-2.5 group">
          ...
          <span className={`text-lg font-bold ${isDark ? 'text-white' : 'text-ink-900'}`}>AI Interview Assistant</span>
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`hidden lg:flex items-center gap-2 rounded-full px-4 py-2 ${
            ...
            <svg className={`w-4 h-4 ${isDark ? 'text-white/50' : 'text-ink-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button className="w-9 h-9 rounded-full bg-primary-500 flex items-center justify-center
